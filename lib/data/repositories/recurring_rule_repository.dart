...
const _log = AppLogger('RecurringRuleRepo');
...
class RecurringRuleRepository with CorruptionTracker {
  ...
  Future<({List<ui.RecurringRule> entities, int corruptedCount})>
      _decryptRows(List<db.RecurringRuleRow> rows) {
    return decryptRowsWithCache<ui.RecurringRule, RecurringRuleData, db.RecurringRuleRow>(
      rows: rows,
      rowId: (row) => row.id,
      rowBlob: (row) => row.encryptedBlob,
      decryptRow: (row) => encryptionService.decryptRecurringRule(
        row.encryptedBlob,
        expectedId: row.id,
